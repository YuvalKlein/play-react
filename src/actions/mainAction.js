const sessionList =
  process.env.NODE_ENV === "development" ? "sLD" : "sessionList";
...
const convertUndefinedToNull = obj => {
  ...
  for (let key of Object.keys(copied)) {
    if (copied[key] === undefined) {
      ...
      copied[key] = convertUndefinedToNull(copied[key]);
    }
  }
...
export const addNewSession = session => {
  return (dispatch, getState, { getFirestore }) => {
    ...
    firestore
      ...
      .add(sessionWithNulls)
      .then(res => {
        console.log("RESS", res);
        dispatch({
          type: "ADD_NEW_SESSION",
          payload: { id: res.id, sessionTitle: session.title }
        });
      })
      .catch(err => {
