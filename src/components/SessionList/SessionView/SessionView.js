...
class sessionView extends React.Component {
  ...
  getPlayersClassName = () => {
	const { players, maxPlayers, minPlayers } = this.props.session;

    if (players.length === parseInt(maxPlayers)) {
      return classes.playersFull;
    } else if (players.length >= parseInt(minPlayers)) {
      return classes.playersApproved;
    }

    return "playersNormal";
  };
    ...
    return (
      <div className={classes.SessionView}>
        ...
        <div
          ...
          onClick={() => this.toggle("toggleInfo")}
        >
          <p className={this.getPlayersClassName()}>
...
export default compose(
  ...
  firestoreConnect([
    {
      collection: process.env.NODE_ENV === "development" ? "sLD" : "sessionList"
    }
  ])
)(sessionView);
